...
if sys.argv[1:] == ['server']:
    ...
    while True:
        ...
        while True:
            ...
            if not message:
                break
            sc.sendall(message.upper()) # send it back in upper case
            n += len(message)
...
elif sys.argv[1] == 'client' and sys.argv[2].isdigit():
    ...
    while True:
        data = s.recv(42)
        if not received:
            print 'The first received says', repr(data)
        received += len(data)
        if not data:
            break
    ...
    print >>sys.stderr, 'usage: tcpdeadlock.py server | client <bytes>'
